    ...
    fetch(`${process.env.REACT_APP_RAGA_URL}/offboarding/getByIds/${ids}`, {
      ...
      .then((result) => {
        ...
        if (isMounted) {
          setData(result);
          fetch(
            `${process.env.REACT_APP_RAGA_URL}/offboardingSession/getMySessions/${result[0].orgID}/${result[0].empID}`,
            {
              headers,
            }
          )
      ...
      .then((result) => {
        ...
        fetch(`${process.env.REACT_APP_RAGA_URL}/appointmentParticipant/add`, requestOptions2)
          ...
          .then((resultr) => {
            ...
            fetch(`${process.env.REACT_APP_RAGA_URL}/offboardingSession/add`, requestOptions)
              .then(async (res) => {
                const aToken = res.headers.get("token-1");
                localStorage.setItem("rexxdex", aToken);
                return res.json();
...
export default OffboardingSession;
