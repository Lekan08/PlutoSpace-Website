...
export default function MyPension() {
  ...
  const handleGets = () => {
    if (start && end) {
      ...
      setOpened(true);
      fetch(
        `${process.env.REACT_APP_JOHANNESBURG_URL}/pensions/getForEmp/${orgIDs}/${pid}?startTime=${strt}&endTime=${ends}`,
        { headers }
      )
        .then(async (res) => {
