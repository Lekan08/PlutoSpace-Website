...
const TaskAudit = ({ taskId }) => {
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    fetch(`${process.env.REACT_APP_HALIFAX_URL}/taskAudit/getForTask/${orgIDs}/${taskId}`, {
      headers,
    })
      .then(async (res) => {
      ...
      .then((result) => {
        ...
        if (isMounted) {
          if (result.length !== 0) {
            fetch(
              `${process.env.REACT_APP_HALIFAX_URL}/taskComment/getForTask/${orgIDs}/${taskId}`,
              {
                headers,
              }
            )
        ...
        <div>
          {taskAudits.map((item) => {
            ...
            return (
              ...
              >
                {item.comment}{" "}
                <div className="row" style={{ marginLeft: "16vw" }}>
