...
export default function SingleCorporateTable() {
  ...
  const handleDisable = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`${process.env.REACT_APP_LOUGA_URL}/corporate/delete/${id}`, requestOptions)
          .then((res) => res.json())
          .then((resx) => {
  ...
  const handleUpdate = (value) => {
    navigate(`/Corporate/update?id=${value}`);
  };

  const handleView = (value) => {
    navigate(`/Corporate/view?id=${value}`);
  };
