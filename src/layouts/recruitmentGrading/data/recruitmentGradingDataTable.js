...
export default function RecruitmentGradingData() {
  ...
  const handleDisable = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`${process.env.REACT_APP_RAGA_URL}/recruitmentGrading/delete/${id}`, requestOptions)
          .then(async (res) => {
            const aToken = res.headers.get("token-1");
            localStorage.setItem("rexxdex", aToken);
            return res.json();
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    fetch(`${process.env.REACT_APP_RAGA_URL}/recruitmentGrading/gets/${orgIDs}`, { headers })
      .then(async (res) => {
        const aToken = res.headers.get("token-1");
        localStorage.setItem("rexxdex", aToken);
        return res.json();
  ...
  return {
    columns: [
      ...
      { Header: "Maximum Score", accessor: "maxScore", align: "left" },
      {
        Header: "Color",
        accessor: "colorCode",
        Cell: ({ cell: { value } }) => <input type="color" disabled value={changeCol(value)} />,
        align: "left",
      },
      {
        Header: "Date Created",
        ...
        Cell: ({ cell: { value } }) => (
          ...
          >
            <Dropdown>
              ...
              <Dropdown.Menu>
                <Dropdown.Item onClick={() => handleShow(value)}>View/Update</Dropdown.Item>
