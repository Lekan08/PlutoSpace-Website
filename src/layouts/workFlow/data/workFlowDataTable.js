...
export default function WorkflowData() {
  ...
  const handleDisable = (value) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`${process.env.REACT_APP_RAGA_URL}/workflow/delete/${value}`, requestOptions)
          .then(async (res) => {
            const aToken = res.headers.get("token-1");
            localStorage.setItem("rexxdex", aToken);
            return res.json();
  ...
  const handleAddStage = (value) => {
    navigate(`/workFlow/add-Stage?id=${value}`);
  };
          ...
          >
            <Dropdown>
              ...
              <Dropdown.Menu>
                ...
                <Dropdown.Item onClick={() => handleAddStage(value)}>Add Stage</Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>
          </div>
        ),
