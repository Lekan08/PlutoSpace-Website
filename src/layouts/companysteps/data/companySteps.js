...
export default function data() {
  ...
  const handleShow = (filteredData, value) => {
    ...
    MySwal.fire({
      title: "Update Company Step",
      html: `<table><tr><td>
      <tr><td><label for="name">Name</label></td>
        <td><input type="text" class="swal2-input" id="name" value="${namex}" placeholder="Name"></td>
`,
  ...
  const handleDisable = (value) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`${process.env.REACT_APP_KUBU_URL}/step/delete/${value}`, requestOptions)
          .then(async (res) => {
            const aToken = res.headers.get("token-1");
            localStorage.setItem("rexxdex", aToken);
            return res.json();
