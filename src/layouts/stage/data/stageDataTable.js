...
export default function StageData() {
  ...
  const handleUpdate = (idx, namex, descripx, createdTimex, deleteFlagx) => {
    ...
    fetch(`${process.env.REACT_APP_RAGA_URL}/stage/update`, requestOptions)
      .then(async (res) => {
  ...
  const handleShow = (filteredData, value) => {
    ...
    MySwal.fire({
      ...
      <td><input type="text" id="name" value="${namex}" class="swal2-input" placeholder="Stage"></td></tr><br>
  ...
  const handleDisable = (value) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`${process.env.REACT_APP_RAGA_URL}/stage/delete/${value}`, requestOptions)
          .then(async (res) => {
            const aToken = res.headers.get("token-1");
            localStorage.setItem("rexxdex", aToken);
            return res.json();
  ...
  return {
    columns: [
      { Header: "stages", accessor: "name", align: "left" },
