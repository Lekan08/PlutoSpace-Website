...
export default function TableData() {
  ...
  const handleDisable = (value) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`${process.env.REACT_APP_LOUGA_URL}/companyServices/delete/${value}`, requestOptions)
          .then(async (res) => {
            const aToken = res.headers.get("token-1");
            localStorage.setItem("rexxdex", aToken);
            return res.json();
  ...
  const handleBranch = (value) => {
    navigate(`/services/branches?id=${value}`);
  };

  // const handleView = (value) => {
  //   navigate(`/products/view-Products?id=${value}`);
  // };
  const handleUpdate = (value) => {
    navigate(`/services/update?id=${value}`);
  };
  // const handleReport = (value) => {
  //   navigate(`/product/Production-report?id=${value}`);
  // };
  // Return table
  return {
    columns: [
      ...
      { Header: "Price Per Unit (NGN)", accessor: "pricePerUnit", align: "left" },
      {
        Header: "Date Created",
          ...
          >
            <Dropdown>
              ...
              <Dropdown.Menu>
                ...
                {/* <Dropdown.Item onClick={() => handleReport(value)}>Production Report</Dropdown.Item> */}
                <Dropdown.Item onClick={() => handleBranch(value)}>Branches</Dropdown.Item>
