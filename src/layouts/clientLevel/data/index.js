...
export default function ClientLevelData() {
  ...
  const handleUpdate = (idx, namex, descripx, createdTimex, deleteFlagx) => {
    ...
    fetch(`${process.env.REACT_APP_LOUGA_URL}/clientLevels/update`, requestOptions)
      .then(async (res) => {
  ...
  const handleDisable = (value) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`${process.env.REACT_APP_LOUGA_URL}/clientLevels/delete/${value}`, requestOptions)
          .then(async (res) => {
            const aToken = res.headers.get("token-1");
            localStorage.setItem("rexxdex", aToken);
            return res.json();
