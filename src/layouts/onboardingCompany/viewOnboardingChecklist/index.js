...
function Checklist() {
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    fetch(`${process.env.REACT_APP_RAGA_URL}/onboarding/getByIds/${ids}`, { headers })
      .then(async (res) => {
        const aToken = res.headers.get("token-1");
        localStorage.setItem("rexxdex", aToken);
        return res.json();
      ...
      .then((resultp) => {
        ...
        if (isMounted) {
          console.log(resultp);
          if (resultp[0].checklists !== null) {
            if (resultp[0].checklists.filter((item) => item.done === false)) {
              setItems(resultp[0].checklists);
            } else {
              setNItems(resultp[0].checklists);
            }
          }
        }
      });
  ...
  const handleOnClick = (e, value) => {
    ...
    fetch(`${process.env.REACT_APP_RAGA_URL}/onboarding/markChecklist`, requestOptions)
      .then(async (res) => {
        const aToken = res.headers.get("token-1");
        localStorage.setItem("rexxdex", aToken);
        return res.json();
              ...
              <FormGroup style={{ padding: "20px" }}>
                {itemsx.map((item) => (
                  <Grid key={item.id} item xs={12}>
                    <FormControlLabel
                      control={<Checkbox />}
                      onClick={(e) => handleOnClick(e, item.checker)}
